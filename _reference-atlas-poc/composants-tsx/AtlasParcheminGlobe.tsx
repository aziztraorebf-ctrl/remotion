...
const KEYFRAMES: CameraKeyframe[] = [
  ...
  { frame: 120, lon: 0,             lat: 16,             zoom: 3.5, pitch: 25, bearing: 5 },
  { frame: 240, lon: TOMBOUCTOU_LON, lat: TOMBOUCTOU_LAT, zoom: 5.5, pitch: 55, bearing: 15 },
];

export const AtlasParcheminGlobe: React.FC = () => {
  const frame = useCurrentFrame();
  const { width, height } = useVideoConfig();
  const containerRef = useRef<HTMLDivElement>(null);
  const mapRef = useRef<mapboxgl.Map | null>(null);
  const [handle] = useState(() => delayRender("Parchemin globe render"));

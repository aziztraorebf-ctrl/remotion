"""
Seedance 2.0 Storyboard-to-Video Test — Soundjata Acte V Segment A (12s).

Tests the voxelplot storyboard-to-video technique:
- 4 refs passed (storyboard + 2 char sheets + environment plate)
- Seedance generates a multi-cut micro-sequence from the storyboard

Segment A covers:
  SHOT 1 (0-3s)    Soumaoro invulnerable, arrows bouncing off
  SHOT 2 (3-6s)    Close-up Soumaoro triumphant
  SHOT 3 (6-9s)    Griot sage whispers secret to Soundjata
  SHOT 4 (9-12s)   Hands binding rooster spur to arrowhead

Audio generation: ENABLED (let Seedance auto-sync SFX on combat motion).
Aspect ratio: 16:9 (we'll crop to 9:16 in Remotion post).
...
def main():
    print("=" * 64)
    print("SEEDANCE 2.0 STORYBOARD-TO-VIDEO - Soundjata Acte V Segment A")
    print("=" * 64)
    ...
    print(f"\n[CALL] {endpoint}")
    print(f"       duration=12s, 720p, 16:9, audio=True")
    ...
    out_path = OUT_DIR / f"storyboard-segment-A-v1-{int(t0)}.mp4"
    print(f"\n[DOWNLOAD] {video_url}")

"""
Generate Soundjata insult clip via Seedance 2.0 image-to-video (fal.ai).

Input  : soundjata-insult-starting-pose-styleref.png (v3 avec sandales)
Output : ~5s clip, dialogue francais audio-guided
Cost   : ~$1.50 (5s x $0.30/s — no video ref)
"""
...
SECONDS 0 TO 2: Steady medium-wide shot, both women in frame. Rival wife's mouth OPENS, finger HOLDS steady toward Sogolon, lips MOVING as she speaks in French: "Au moins mon fils peut cueillir des feuilles de baobab." Sogolon's chin LIFTS slightly, eyes NARROW, jaw CLENCHES.

SECONDS 2 TO 4: Camera holds steady. Rival wife continues: "Le tien ne sait meme pas se lever." Her lip CURLS in contempt. Sogolon's LEFT hand CLENCHES into a fist, breath CATCHES.

SECONDS 4 TO 5: Rival wife's arm LOWERS slowly to her side, expression settling into cold satisfaction. Sogolon stands motionless, head high.
...
def main():
    ...
    print("=" * 60)
    print("Soundjata — Insult clip generation (Seedance 2.0 i2v)")
    print("=" * 60)
    print(f"Endpoint : {ENDPOINT}")
    print(f"Duration : 5s")
    print(f"Resolution: 720p, 9:16")
    print(f"Cost est : ~$1.50")
    ...
    args = {
        ...
        "aspect_ratio": "9:16",
        "resolution": "720p",
        "generate_audio": True,  # besoin du dialogue francais
    }
    ...
    print("Waiting for result (typically 60-180s)...\n")
    ...
    # save raw result
    result_path = OUTPUT_DIR / f"result-{int(time.time())}.json"
    ...
    output_video = OUTPUT_DIR / "soundjata-insult-v1.mp4"

...
Usage:
  python3 scripts/tools/svg-scene-narrative.py --provider gemini|gpt --brief "<scene>" \
    [--narrative-ref scene.png] [--style-ref encre.png] [--ratio 9:16|16:9] --out /tmp/x.json
...
- de la profondeur (avant-plan / fond), un cadrage qui remplit le {RATIO_LABEL}.
...
def build(brief, has_narr, has_style, ratio):
...
def gen_gemini(brief, narr, style, out, ratio):
    from google import genai
    from google.genai import types
    client = genai.Client(api_key=os.getenv("GEMINI_API_KEY"))
    parts = [types.Part.from_text(text=build(brief, bool(narr), bool(style), ratio))]
    for img in _imgs(narr, style):
        parts.append(types.Part.from_bytes(data=Path(img).read_bytes(), mime_type="image/png"))
    print(f"[gemini] {GEMINI_MODEL} svg-narrative ratio={ratio} ({len(_imgs(narr,style))} ref) ...")
...
def gen_gpt(brief, narr, style, out, ratio):
    import requests
    ...
    print(f"[gpt] {GPT_MODEL} svg-narrative ratio={ratio} ({len(_imgs(narr,style))} ref) ...")
...
def main():
    ...
    ap.add_argument("--ratio", default="9:16", choices=["9:16", "16:9"])

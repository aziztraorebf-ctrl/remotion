"""Pecheur test v3 — Sequence complete, point de depart propre — Grok Imagine Video 1.5.
Duree : 10s | Resolution : 720p | Cout ~= 10 * $0.14/s = $1.40
Corrige v1/v2 : repart d'une image ou le filet est DEJA en main (generee via Gemini i2i),
panier unique deja vide -- le modele n'a plus a halluciner l'apparition du filet ni un 2e panier.
Prompt = sequence complete (lancer->ramener->tourner->poser->regard) + clause anti-duplication.
"""
...
SOURCE_IMAGE    = REPO / "public/assets/pecheur-grok-test/scenes/pecheur-etat-A-filet-plie.png"
OUTPUT_DIR      = REPO / "public/assets/pecheur-grok-test/clips"
OUTPUT_MP4      = OUTPUT_DIR / "pecheur-grok-v3.mp4"
OUTPUT_META     = OUTPUT_DIR / "pecheur-grok-v3.meta.json"
REQUEST_ID_FILE = OUTPUT_DIR / "pecheur-grok-v3.request-id.txt"
...
RESOLUTION    = "720p"
ASPECT_RATIO  = "16:9"
COST_ESTIMATE = 1.40   # 10s * $0.14/s (720p)

# v3 : sequence complete, mais point de depart deja correct (filet en main, panier unique vide)
# -- le modele n'a plus a inventer l'apparition du filet ni dupliquer un panier.
...
def download_and_save(request_id, data):
    ...
    OUTPUT_MP4.write_bytes(r.content)

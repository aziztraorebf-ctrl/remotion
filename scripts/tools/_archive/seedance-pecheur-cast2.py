"""Pecheur test — Cast 2 (2e lancer) — Seedance 2.0 image-to-video.
...
SOURCE_IMAGE    = Path("/tmp/pecheur-cast2-source.png")
OUTPUT_DIR      = Path("/tmp")
OUTPUT_MP4      = OUTPUT_DIR / "pecheur-seedance-cast2.mp4"
OUTPUT_META     = OUTPUT_DIR / "pecheur-seedance-cast2.meta.json"
REQUEST_ID_FILE = OUTPUT_DIR / "pecheur-seedance-cast2.request-id.txt"
...
The fisherman stands in his pirogue, a large industrial trawler now visible looming on the horizon behind him. He performs one full fishing sequence, narrated as continuous action: He WINDS UP his cast, torso leaning back, arm raised high with the folded net. He RELEASES the throw — the net WHIPS forward and fans open over the water. He HAULS the net back in with visible effort — this time the net comes back with only one or two fish, noticeably lighter than before. He TURNS in a three-quarter angle toward the inside of the boat — never facing the camera directly — and LOWERS the fish into the wicker basket resting at the bottom of the pirogue. He STRAIGHTENS, gaze drifting toward the looming trawler on the horizon. His expression grows more tense, brow lowering slightly.

Identity lock: dark brown skin, teal shirt, dark slate-grey pants, shaved head, calm profile face, same proportions throughout. Pirogue, wicker basket, and flat ink ocean/sky stay consistent with the source image. The trawler on the horizon stays in its position, do not move it closer or animate it independently.

...
| ⭐ **Reproduire un OBJET depuis une image de référence, brief LIBRE** | `scripts/tools/svg-from-ref-image.py` | `--provider gemini\|gpt\|kimi\|grok --brief f.md --ref img.png --out x.svg`. Le brief est FOURNI (fichier .md), pas verrouillé dans le script. ⚠️ Ne pas confondre : `svg-from-image-target.py` a son brief INTERNE figé sur le registre gravure/parchemin 9:16 · `llm-gen-svg.py` est figé sur des jetons, text-only. Sert le contrat « prêt à animer ». Prouvé 2026-08-22 (4 modèles, même objet). |
...
| **Choisir/importer un identifiant NON-Gemini** (voix ElevenLabs, Kimi, GPT/OpenRouter, GLM, DeepSeek, Minimax music) | `scripts/tools/api_models.py` | Module, **SOURCE DE VÉRITÉ UNIQUE** — ⛔ ne JAMAIS ré-écrire ces chaînes en dur. `from api_models import VOICE_SOUVERAIN, KIMI_REVIEW, ...` ⚠️ Migration en cours : ~75 fichiers existants pas encore migrés, mais **tout NOUVEAU script importe d'ici**. |
...
| **Vérifier MÉCANIQUEMENT une frame carto avant de la présenter ⭐** | `scripts/tools/carto-selfreview.py` | `--frame f.png [--globe-interdit\|--globe-attendu] [--attendu-fond RRGGBB] [--sujet-min N --sujet-max N]`. O/X sur 4 critères (projection, fond vs palette, occupation du cadre, vide), **exit 1** si un critère dur échoue. Né de 3 défauts qui ont traversé plusieurs rendus sans être vus (globe silencieux de Mapbox, fond `dark-v11` brut, cadrage jugé à l'œil). ⚠️ `--attendu-fond` ne vaut QUE si le fond est visible aux bords : sur une vue pleine il mesure les terres. |
| **Comparer un rendu à SA case de storyboard ⭐** | `scripts/tools/make-comparatif-panel.py` | `--storyboard SB.jpg --row 0\|1 --panel 1-4 --render frame.png --out cmp.png`. UNE case ↔ UNE frame **pleine taille**, même hauteur. ⛔ Ne JAMAIS empiler des vignettes : Gemini ET Grok ont halluciné « le pays occupe 15-20 % du cadre » (mesure réelle : 61 %). |
| Générer le GeoJSON admin-1 d'un pays (Natural Earth 10m) | `scripts/warmap/generate-zambia-admin1.py` | Modèle à copier pour un autre pays (le Sahel a `generate-sahel-admin1.py`). Porte la contrainte client dans la DONNÉE (`inBrief`) : une entité interdite n'est pas exposée à la scène. |
...
| Éprouver un hook/gate dans les 2 sens (doit bloquer ET doit se taire) | `scripts/tools/test-gate.py` | ⛔ Un gate jamais vu se déclencher est indiscernable d'un gate mort. Passe les cas par stdin, donc testable même quand la commande contient le motif surveillé. |

...
const PROMPT = `Generate a pixel art image in 16-bit SNES/GBA style.
...
async function main(): Promise<void> {
  ...
  console.log("=== Nano Banana Pro (Gemini 3 Pro Image Preview) ===");
  ...
  for (const part of parts) {
    if (part.inlineData) {
      ...
      const destPath = path.join(OUTPUT_DIR, `peste-pixel-NANOBANANAPRO.${ext}`);

      const buffer = Buffer.from(base64Data, "base64");
      fs.writeFileSync(destPath, buffer);
      const size = (buffer.length / 1024).toFixed(0);
      console.log(`Saved: peste-pixel-NANOBANANAPRO.${ext} (${size} KB)`);
      imageFound = true;
    } else if (part.text) {
  ...
  if (!imageFound) {
    ...
    console.error(JSON.stringify(parts.map((p: any) => Object.keys(p)), null, 2));
  }
}

...
Scans public/assets/abou-bakari/ and public/audio/abou-bakari/ to detect:
- Scene image sources (scene-id-source-v*.png)
- Scene clip finals (sX-*.mp4)
- Updates data/scenes.json status field and paths

Usage:
  python3 abou-bakari-dashboard/scripts/sync-dashboard.py
...
DATA_FILE = ROOT / "abou-bakari-dashboard" / "data" / "scenes.json"
ASSETS_ROOT = ROOT / "public" / "assets" / "abou-bakari"
...
def find_latest_clip(scene_id: str) -> tuple[str | None, float | None]:
    ...
    # Match patterns like s1-ocean-v1.mp4, fleet-v2.mp4, etc.
    base_id = scene_id.replace("scene", "s")
    candidates = sorted(
        [*clips_dir.glob(f"{base_id}-*.mp4"), *clips_dir.glob(f"*-{base_id}-*.mp4")],
        key=lambda p: p.stat().st_mtime,
        reverse=True,
    )
    for candidate in candidates:

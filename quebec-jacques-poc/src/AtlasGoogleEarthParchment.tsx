...
const stripAllLabels = (map: mapboxgl.Map) => {
...
export const AtlasGoogleEarthParchment: React.FC = () => {
  const frame = useCurrentFrame();
  const { width, height } = useVideoConfig();
  const containerRef = useRef<HTMLDivElement>(null);
  const mapRef = useRef<mapboxgl.Map | null>(null);
  const [handle] = useState(() => delayRender("Google Earth parchment-tinted render"));
    ...
    <AbsoluteFill style={{ backgroundColor: "#000" }}>
      <div
        ...
        style={{
          ...
          top: 0,
          left: 0,
          filter: "sepia(0.55) saturate(1.4) hue-rotate(-10deg) brightness(1.05) contrast(1.1)",
        }}
      />
      <AbsoluteFill
        style={{

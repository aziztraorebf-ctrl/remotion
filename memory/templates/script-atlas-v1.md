...
**Anti-pattern absolu Atlas** : "Le pétrole du Sénégal", "L'uranium nigérien" — titre descriptif = invisibilité algorithmique + double audience ratée.

**Exemples Atlas fusionnant les deux couches** : "Le Niger recevait 9 centimes sur l'euro depuis 53 ans" ✅ — "Le pays qui a inventé le paiement mobile avant Apple" ✅ — "Le Maroc détient 70% des batteries de demain" ✅
...
- `memory/tools/atlas-camera-movements.md` — mouvements caméra Remotion Atlas (frame-driven, zéro coût par essai)
- `memory/tools/camera-movements.md` — 30 mouvements caméra Seedance/Kling/Veo (génération vidéo AI, NE PAS utiliser pour Remotion)
- Note : `atlas-camera-movements.md` = Remotion code ; `camera-movements.md` = Seedance/AI prompts — deux domaines distincts

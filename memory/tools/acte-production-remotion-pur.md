# Workflow de production d'un acte Remotion pur — Pattern valide

> Migré depuis auto-memory 2026-08-31. Etabli 2026-04-13 sur Soundjata Acte VI (Charte du Manden, 20.5s).
> Pattern pour créer un acte **100% Remotion** sans clip video — uniquement narration audio +
> composition animée avec assets Gemini + texte typographique. Distinct des pipelines Seedance
> (`memory/tools/seedance-rules.md`) et du pipeline carte historique (`memory/archive/historical-map-pipeline.md`).
...
Ne PAS utiliser whisper local — lent sur Mac Apple Silicon meme avec small/medium. L'API est 100x plus rapide et gratuite a cette echelle. (Rappel doctrine projet : Whisper API seulement, jamais en local — cf CLAUDE.md gates.)
...
Modele : `IMAGE_MODEL` (defaut Lite — asset de production detoure/composite ensuite, pas un livrable publie ; ⛔ importer depuis `scripts/tools/gemini_models.py`, jamais en dur ; `IMAGE_MODEL_HQ` seulement si l'image est publiee telle quelle)
...
Une fois generes pour un episode, ces assets sont reutilisables pour d'autres episodes de la meme serie :

- `parchment-bg.png` — fond sobre universel
- Icones symboliques (vie, protection, dignite) — reutilisables entre episodes
- Medaillons — souvent specifiques a un episode

...
95. **Reference-to-video VALIDE pour pixel art 16-bit (style Atlas PixelLab)** : contrairement au paper-craft (regle 83/86 = echec), le style pixel art 16-bit est suffisamment distinct pour que reference-to-video le preserve. Test Hannibal Beat 2 (2026-05-05) : 5 refs PixelLab (Hannibal, elephant-radeau, Volque, background Rhone) → style pixel art maintenu, personnages reconnaissables, eau animee, mise en scene correcte (Hannibal rive gauche, elephant sur radeau, Volques falaise droite). **Conditions de succes** : (1) background Gemini dans le meme style pixel art comme ref centrale, (2) sprites PixelLab PNG (rotations) comme refs personnages, (3) prompt SECONDS X TO Y conçu pour Seedance (pas un brief Remotion recycle). **Attention** : test 1 utilisait un prompt de brief Remotion comme prompt Seedance = resultat sous-optimal. Le prompt doit etre conçu pour Seedance des le depart.

96. **Reference-to-video : prompt Remotion recycle ≠ prompt Seedance** : le brief technique Remotion (frame counts, composants, layers) est inutilisable directement comme prompt Seedance. Le prompt Seedance doit decrire une ACTION cinematographique continue sur 10-15s : exit-frame radeau, sky animation, camera movement. Validation semi-positive beat2-r2v-test1.mp4 (2026-05-05) mais prompt inadapte = animation elephant quasi-statique + radeau arrive sur rive au lieu de sortir du cadre. Prochaine iteration : prompt conçu pour Seedance avec Format 3 SECONDS + exit-frame explicite + ciel anime.

97. **Storyboard panels comme refs Seedance = RISQUE : panels s'animent eux-memes** : fournir un storyboard (grille de frames staticues) comme ref image a reference-to-video = Seedance traite les PANELS comme du contenu a animer. Il peut animer les bordures du storyboard, faire entrer en mouvement les cases elles-memes, ou interpoler bizarrement entre les cases adjacentes. **Alternative validee** : extraire chaque panel en image individuelle et n'envoyer QU'UN panel a la fois (le plus representatif de la scene voulue), pas la grille complete. A tester systematiquement avant d'envoyer storyboard en ref. Source : retour session Hannibal Beat 2 2026-05-05.

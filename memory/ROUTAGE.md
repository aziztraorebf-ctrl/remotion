...
| **Scène NARRATIVE avec un PERSONNAGE humain** (marcher, porter, échanger un objet, foule, attendre, héler) | `src/projects/_shared/stick-figure-svg/STICK-FIGURE-INDEX.md` ⭐⭐ — repo principal (le worktree `remotion-cfa` n'existe plus, vérifié 2026-09-11). Registre EN PRODUCTION depuis le 2026-07-28. Socle `StickFigure.tsx` à IMPORTER. Y sont gravés le **FILTRE DE SCÈNE** (≥2 « non » = refuser la scène), les 6 briques et les pistes écartées. |
...
| Sujet | Fichier / outil |
|---|---|
| **⭐⭐ TROUVER LE TITRE d'une vidéo (longue, Short, caption)** | `scripts/tools/jury-titres-llm.py <script.md> --contexte "..."` — jury 4 modèles (Kimi + Gemini + GPT-5.5 + **Grok**), 10 titres classés chacun, ~2 min. ⛔ **Ne JAMAIS générer ET juger un titre soi-même** (juge et partie : 3 séries rejetées + 1 titre factuellement faux le 2026-07-30). Le signal = la CONVERGENCE entre modèles. ⚠️ recompter les caractères soi-même (limite 55). ⭐⭐ **Doctrine complete (dont CE QU'IL FAUT INJECTER dans le --contexte) : `memory/doctrines/PACKAGING-YOUTUBE.md` §4** |
| **⭐⭐ CONCEVOIR UNE MINIATURE** | `scripts/tools/jury-thumbnail-llm.py <script.md> --contexte "..."` → 5 concepts SVG classés, PUIS composer le SVG soi-même (ou agent Fable). ⛔ **Une miniature EST une scène SVG statique — on la COMPOSE, on ne la génère pas par IA** (texte exact avec accents, registre cohérent avec la vidéo, coût nul). Vérifier à 320 px et REGARDER. ⭐⭐ **Doctrine complete : `memory/doctrines/PACKAGING-YOUTUBE.md` §5** |
| ⭐⭐⭐ **PACKAGING (titre · miniature · description · engagement) — SOURCE DE VERITE UNIQUE** | `memory/doctrines/PACKAGING-YOUTUBE.md` — absorbe les 6 anciens fichiers epars (regles-titres, doctrine-titres, jury-titres, thumbnail-svg, pipeline-c, verifier-style). ⭐ **Le principe maitre : le TITRE cree le manque, l'IMAGE donne l'emotion, le CONTENU resout** (§1). Regles mesurees §3, jury §4, miniature §5, engagement §6. |
...
| **EXTRAIRE quoi que ce soit de YouTube** (transcript, frames, audio, métadonnées) | `memory/tools/yt-dlp.md` ⭐⭐ ⛔ JAMAIS scraper à la main. **Lire ce fichier AVANT de tâtonner** : les gotchas 403/429 y sont déjà résolus (`--extractor-args "youtube:player_client=mweb"` ou `android` ; ⛔ `web`/`tv` échouent). Re-découvert par tâtonnement le 2026-08-20 alors que le fix y était depuis le 08-18 — le fichier était bon, il n'était simplement routé nulle part. |
...
| ⭐⭐⭐ **CHOISIR LA FORME NARRATIVE (avant d'écrire le script)** | **`memory/doctrines/FORMES-NARRATIVES.md`** — 7 formes (bascule · question non résolue · révision du récit officiel · enquête sur du froid · scène qui contient le problème · deux trajectoires · boucle/paliers), conditions d'usage + arbre de décision + dosage mesuré (densité verbale, relances). **Le chaînon qui manquait : la forme était héritée par défaut (= documentaire explicatif), jamais choisie.** Se pose AU SCRIPT, juste avant le moteur visuel. |
...
| **Structurer le CORPS d'un script (squelette narratif objet→mécanisme, après le sujet validé)** | `memory/doctrines/STRUCTURE-OBJET-MECANISME.md` — 5 temps (objet concret → question qui organise → cascade causale → reveal chiffré → clôture humaine), décodé AJ+ français. Distinct de `HOOK-PREMIERE-MINUTE.md` (1ère minute) et `SUJET-PRIME-SUR-PRODUCTION.md` (validation sujet, en amont). |

...
> - **ACTE 4 « qui tient la clé » — EN PAUSE (2026-07-24)** : décision Aziz — direction pas stabilisée après plusieurs itérations (carte D3 seule → jugée trop proche de l'Acte 2 → tentative split-screen Paris/CFA → aucune option Tour Eiffel/Seine ne convainc). Repris session future. Code en chantier (non commité) : `CfaActe4Cle16x9.tsx`. Géodata Seine réelle préparée (Natural Earth 10m, croppée Paris) : `seineParisGeo.json`. Ce qui MARCHE déjà et peut être repris tel quel : Beat 1 (garantie qui descend, carte D3 recentrée Paris-dominant) + scène encre/nuit 1994 (murs FMI/France + signatures qui s'écrivent seules) — direction validée par comparatif 4 modèles, cf composant.
> - **⛔ SOURCE UNIQUE DU SCRIPT COMPLET (8 beats) = `SCRIPT-MIDFORM-V2.md`** (même dossier) — PAS `SCRIPT-V6.md` (ça c'est la version SHORT 2:20, différente). Nos "Actes" 1-4 en prod = Beats 1-4 du V2. **Beat 5 = scindé en 5a+5b** (décision Aziz 2026-07-21, "une idée = une scène") : **5a "Le quotidien"** (scène Dakar, braise-or/encre chaude, PAS d'humain, riz importé/dollar) · **5b "Le levier perdu"** (registre ⭐À TESTER "salle de contrôle monétaire"/data-terminal néon : manette "TAUX DE CHANGE" active pour un pays normal, verrouillée pour la zone CFA). Puis **Beat 6a/6b** (carte AES + conclusion "qui paie le prix") — PAS encore produits.
>

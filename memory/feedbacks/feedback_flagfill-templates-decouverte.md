...
**Technique clé ImageProjectionFill :** charge image → `bichromize()` (luminance ITU-R BT.709 → interp navy↔gold + contraste) → pushCanvas fill-pattern. Asset gen via `scripts/tools/gemini-gen-image.py` (text-to-image, modèle `IMAGE_MODEL` — defaut Lite ; une texture bichromisee ensuite n'a pas besoin du HQ). 1er asset : `public/_shared/refs/textures/khouribga-mine-satellite.png` (vue satellite mine phosphate). Preview : https://files.catbox.moe/7opcc9.mp4

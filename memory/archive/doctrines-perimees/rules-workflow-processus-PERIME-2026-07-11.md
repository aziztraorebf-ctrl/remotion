# Workflow & Processus — Règles transversales (consolidé) — ⛔ ARCHIVÉ 2026-07-11

> **PÉRIMÉ — ne plus lire ni router.** Contenait un jury LLM (GPT-4o + Grok, section 1) contredisant les modèles verrouillés actuels et le système `DA-BRIEF-GATE.md` réellement utilisé. Numérotation de sections dupliquée (2 fois section 6 et 7), signe de fusion mal faite.
>
> Contenu valide extrait et rapatrié avant archivage (audit multi-agents 2026-07-11) :
> - Sections 2/3/8 (collaboration, profil Aziz, durée Shorts 3min) → `memory/rules-souverain-editorial.md` §10
> - Section 6b (architecture 3 formats) → `memory/rules-souverain-editorial.md` §11
> - Section 4 (critères go/no-go sujets) → doublon strict de `rules-souverain-editorial.md` §8, supprimé sans rapatriement
> - Section 6 (1 session = 1 beat, Atlas) → `memory/rules-atlas-production.md`
> - Section 7 (règles audio silence/overlap/duration) + R-INTEGRATE-BEFORE-ADD → `memory/rules-beat-production.md` R13/R14
> - Sections 1 (jury LLM périmé), 5 (skills preprod, doublon ROUTAGE.md), 9 (checklist Souverain, doublon skill `souverain-preproduction`), 10 (pré-prod humain, doublon DA-BRIEF-GATE) → non rapatriées, obsolètes ou dupliquées ailleurs.
>
> Ligne de routage retirée de `memory/ROUTAGE.md`.

...
| **Path/route dashed qui se DESSINE ville→ville + city markers (#96 Silk Road dorée, #268 sépia, #256 vol)** | FiberOpticBorderDraw dessine une FRONTIÈRE, pas un path point-à-point | ⛔ **VRAI GAP = GeoFlowConnection** (déjà backloggé NEXT-ACTION) |
| **Marqueur animé (avion/icône) le long du path + camera follow (#256)** | aucun | ⛔ gap, extension de GeoFlowConnection |

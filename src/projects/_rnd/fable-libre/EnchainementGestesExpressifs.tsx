...
//
// --------------------------------------------------------------------------------------------
// ⚠️ DEFAUT RESIDUEL CONNU ET NON CORRIGE (assume, mesure, a trancher par Aziz)
// --------------------------------------------------------------------------------------------
// La scene se termine sur le corps EN TRAIN de basculer, pas pose au sol. Dans cette pose finale,
// les PIEDS passent legerement SOUS la ligne de sol : mesure, pied1 a +6.95u (18.1px) et pied2 a
// +2.70u (7.0px) sous le sol.
...
// → Question ouverte pour Aziz, pas un choix a faire seul dans un banc d'essai.
//
...
// ============================================================================================
// LE MOTEUR <Stick> — RECOPIE CONFORME de GestesExpressifs16x9
// ============================================================================================
...
// dans le registre. Ici tout le fichier vit dans CETTE convention, sans exception : c'est
// justement ce qui permet de n'avoir aucun raccord de convention a gerer.
type PoseStick = {
  bob: number;
  leanBuste: number;
  leanTete: number;
...
const POSE_NEUTRE: PoseStick = {
  ...
  brasAvantCoude: 8,
  brasArriereCoude: 8,
  // ⚠️ jamais 0/0 : deux jambes au meme angle = un seul trait a l'ecran (regle dure du registre).
  jambeAvant: 5,
  jambeArriere: -5,
  jambeAvantGenou: 0,
  jambeArriereGenou: 0,
...
//   D (effondrement) 1.5s — DERIVE, pas choisi : le spring de chute est encore en mouvement
//                          jusqu'a la frame ~27 (0.90s — mesure : spring >= 0.995), puis on
//                          laisse 0.6s de pose tenue, ce qui joue le role du temps d'inertie du
//                          geste valide (l'encaissement). ⛔ Les valeurs precedentes (4.4s puis
//                          3.0s) laissaient 2.5-3.5s d'image STRICTEMENT FIGEE en fin de scene
//                          (defaut mesure au render v2) : une pose tenue n'est pas un geste.
const A_DUR = S(3.2);
const B_DUR = S(5.4);
const C_DUR = S(4.6);
const D_DUR = S(1.5);
...
export const EXPRESSIFS_FRAMES = D_START + D_DUR;
...
// ⛔ RECOPIEES de BandeChute, valeur pour valeur (convention <Figure> : 0 = membre qui PEND).
const P_CHUTE: PoseFigure = {
  hipY: -20, torso: 58, leg1: -30, leg2: -52, knee1: -14, knee2: 8,
  arm1: 74, arm2: 88, arm1Len: 29, arm2Len: 28,
};
// ⛔ P_SOL (le corps a plat) est VOLONTAIREMENT ABSENTE : demontre insoluble sur ce moteur
// (aucune position de coude ne degage la tete — cf. le long § ci-dessus). La chaine s'arrete a
// P_CHUTE. Ne pas la reintroduire sans avoir leve la contrainte de longueur de bras.

// Position de main que la pose produit SUR <Figure> — de la geometrie, pas une convention.
...
// IK 2 segments (loi des cosinus) dans la convention de <Stick> : trouve (racine, flexion) qui
// place l'extremite sur la cible. ⛔ Brique n°2 du registre, appliquee telle quelle. On borne la
// distance a 98.5% de la portee : le registre interdit le bras en butee (il glisse et tremble).
const ikStick = (
...
// Porte une pose-cle <Figure> vers <Stick> : correspondance demontree pour le corps, IK pour les
// bras. ⛔ Aucun angle de bras n'est recopie ni retraduit.
const porterVersStick = (P: PoseFigure): PoseStick => {
  ...
  // ⭐ SENS DE COUDE (s1 = +1, s2 = +1) : RESOLU, PAS CHOISI. Les 4 combinaisons possibles ont ete
  // evaluees sur la pose portee, avec pour critere la distance coude<->tete (le crane fait 9u de
  // rayon : en dessous de ~13u, le bras traverse la tete et la silhouette se ferme en losange —
  // defaut VU au 1er render). Seule (+1, +1) degage les deux coudes : 13.8u et 18.4u.
  // ⛔ Le sens -1 sur le bras arriere (ce qui semblait "logique" par symetrie avec le miroir de
  // <Stick>) place le coude a 3.1u de la tete, c.-a-d. DEDANS.
  ...
  return {
    ...
    // Stick : racine du bras AVANT = brasAvantAngle + leanBuste
    brasAvantAngle: k1.racine - leanBuste,
    brasAvantCoude: k1.flexion,
    // Stick : racine du bras ARRIERE = -brasArriereAngle + leanBuste, et la flexion est niee
...
const K_CHUTE = porterVersStick(P_CHUTE);

// Fondu terme a terme entre 2 poses-cles.
// ⚠️ CE N'EST PAS UN "FONDU DE JONCTION" (l'anti-pattern proscrit par la brique n°7). C'est la
// brique n°4 du registre : "POSES-CLES EXPLICITES + fondu SEQUENTIEL", la methode meme du geste
// de chute valide. La difference est essentielle : ici on interpole entre 2 poses-cles A
// L'INTERIEUR d'un seul geste (a tout instant on est sur le segment entre 2 silhouettes valides,
// donc jamais de derive) ; l'anti-pattern, lui, melange les poses de DEUX GESTES DIFFERENTS de
// part et d'autre d'une frontiere, ce qui affiche la moyenne de 2 instants dont aucun n'est un
// vrai moment du mouvement. Aucune opacite n'est jamais melangee : un seul corps est dessine.
const mixPose = (A: PoseStick, B: PoseStick, u: number): PoseStick => ({
...
// lent : a stiffness 130 la chute etait bouclee en 5 frames et le reflexe parachute ne se VOYAIT
// pas). La chaine, elle, s'arrete a P_CHUTE au lieu de continuer vers P_SOL — limitation
// demontree, pas un choix de confort (cf. le § "LIMITE TROUVEE AU RENDER" plus haut).
//
// ⚠️⚠️ 2e DEFAUT VU AU RENDER (v2), consequence directe de cette suppression — et corrige ici :
// j'avais conserve la borne `[0, 0.55]` du geste valide, qui est la borne d'ARRIVEE SUR P_CHUTE
// dans une chaine ou P_SOL prend ensuite le relais sur `[0.4, 1]`. En retirant P_SOL, cette borne
// coupait la MOITIE du mouvement : MESURE, versChute atteignait 1.0 des la frame 10, puis l'image
// etait STRICTEMENT FIGEE pendant 2.67s (delta/frame = 0.0000). Au rendu, le corps partait en
// diagonale et restait suspendu en l'air — il ne s'effondrait pas, il se garait.
// ⭐ LE FIX : le spring pilote l'arrivee sur P_CHUTE sur SA PLAGE COMPLETE [0, 1]. On n'invente
// aucune courbe et on ne re-cadence rien — on utilise la courbe du geste valide de bout en bout,
// au lieu de n'en garder que le premier tiers. Le mouvement reste alors vivant jusqu'a la frame
// ~27 (0.90s, mesure : spring >= 0.995), ce qui EST la duree reelle de ce geste.
const poseEffondrement = (f: number, fps: number, debut: PoseStick): PoseStick => {
  const chute = spring({
    frame: f, fps,
    config: { mass: 2.2, damping: 20, stiffness: 62 },
  });
  return mixPose(debut, K_CHUTE, Math.min(1, chute));
};

// ============================================================================================
// LA SCENE
// ============================================================================================
export const EnchainementGestesExpressifs: React.FC = () => {
  ...
  const op = interpolate(frame, [0, 14, EXPRESSIFS_FRAMES - 18, EXPRESSIFS_FRAMES],
    [0, 1, 1, 0], clamp);
...
export default EnchainementGestesExpressifs;

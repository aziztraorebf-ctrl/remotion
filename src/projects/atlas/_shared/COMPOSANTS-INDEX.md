...
| `AtlasV2ConfrontationScene` ⭐ | _ref/scenes/AtlasV2ConfrontationScene | "deux acteurs convergent en SENS OPPOSÉS et se font face" — 2 sprites simultanés (A walk east, B walk west frames NATIVES zéro flip), point de rencontre sur waypoint natif, bump impact au clash, cartouche + Spotlight Insert chiffré (rapport de forces) qui dim la carte. Template "acteurs simultanés" réutilisable pour toute action parallèle. Validé Aziz 2026-06-04. catbox 736mwf |
| `AtlasV2ArmyDeployScene` ⭐⭐ | _ref/scenes/AtlasV2ArmyDeployScene | "DEUX ARMÉES s'affrontent" — LE template bataille complète. Chorégraphie multi-sprites (4v4) en phases : files indiennes (Recette A x2) → déploiement file→ligne (direction déduite du dx, vague par rang) → Spotlight Insert → CHARGE (ruée) → estoc spear_attack PLAY-ONCE (séquentiel ou simultané via `clashMode`) → PERTES (camp perdant tombe `death` + fade-out). SFX charge+clash. Props : perSide, clashMode, losingSide, casualties. Anims PixelLab : walk/charge/spear_attack/death (soldat-mali, 4 dirs). Validé Aziz 2026-06-04 ("ne pas perdre ça"). catbox 2fycin |

...
| **⭐⭐ CARTE ASSOMBRIE + ÉLÉMENTS SUPERPOSÉS** (montrer un CONCEPT/accord/data SANS quitter la carte : on l'assombrit, ses contours/couleurs restent visibles, on superpose sceau/drapeaux/titre/schéma par-dessus = spotlight cinématographique — **TEMPLATE PRINCIPAL validé Aziz 2026-06-14, réutilisable largement**) | `WarMapDimmedOverlay` + helper `dimmedOverlayHole()` | `warmap/_shared/WarMapDimmedOverlay.tsx` | `frame`, `inAt`, `outAt`, `width`, `height`, `veil` (0.62), `haloColor`, `centerY`, `children` (la scène superposée). ⚠️ Si contours rendus par le moteur APRÈS l'overlay → percer un TROU (mask SVG) sous l'élément via `dimmedOverlayHole()` (réf : mask `confed-seal-hole` dans SahelWarMapEngine). NE PAS masquer TOUS les contours. Réf : confédération AES P4 (`p4-chantier3-confed-FINAL.mp4`, catbox xt8ztb). Doctrine : `WARMAP-CARTE-VS-OVERLAY.md`. |
| **⭐⭐ SPLIT-SCREEN — 2 mondes côte à côte** (montrer une DIVERGENCE / comparaison / dépendance : ex. carte AES \| data Paris. Incarne la dualité au lieu de la décrire. **TEMPLATE PRINCIPAL validé Aziz 2026-06-15**, promu des protos R&D P5/P6) | `WarMapSplitScreen` | `warmap/_shared/WarMapSplitScreen.tsx` | `frame`, `inAt`, `outAt`, `width`, `height`, `orientation` ("vertical" gauche\|droite / "horizontal" haut/bas), `ratio`, `panels` (2 render-props `(w,h)=>node`, chacun son repère 0..w/0..h), `labels`, `connector` (render-prop qui TRAVERSE la séparation, ex. fil de parité), `sepColor`. Ouverture animée (volets glissent). Réf : CFA P4 (`p4-cfa-FINAL.mp4`, catbox 5fxlvp) — gauche carte AES+pièce CFA pulsante, droite drapeau FR SVG ondulant + équation "1€ = ~656 FCFA" + bascule vers le SENS en typewriter. Doctrine : `WARMAP-CARTE-VS-OVERLAY.md` (le split = "les deux en parallèle"). |

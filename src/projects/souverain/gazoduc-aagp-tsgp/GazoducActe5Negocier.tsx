// MOTEUR: objet/metaphore SVG — deux tuyaux identiques dont seule l'extremite raconte tout : papiers
// en apesanteur (la parole) contre meche qui perce le sol (l'acte). L'opposition du script se lit
// sans carte, sans texte et sans personnage.
//
// GazoducActe5Negocier — Acte 5, segment 3 "LA CHUTE" (29.041 -> 42.081s absolu, 391 frames).
// « Personne ne sait encore lequel de ces deux traces ira jusqu'au bout. Peut-etre qu'aucun des deux
//   ne verra jamais la Mediterranee. Mais une chose est sure : pendant que certains negocient encore
//   pour savoir qui va payer... »
//
// Dessin+animation : agent Fable 5, puis AMELIORE par une 2e passe ou on lui a renvoye son propre
// rendu + une cible enrichie (methode d'Aziz) — objets varies (documents/billets/sceaux), sol
// texture, tuyaux modeles. Retenu par Aziz au comparatif contre la version Gemini.
//
// ⭐ RECALAGE (2026-08-17) : le prototype durait 450f sans ancre audio. Recale ici sur le forced-align
// reel, frames LOCALES au segment :
//   f194  "Mediterranee" (35.501s) — le doute : aucun des deux n'y arrivera peut-etre
//   f261  "sure"         (37.741s) — la bascule : "mais une chose est sure"
//   f322  "negocient"    (39.781s) — les papiers s'agitent (la parole s'emballe)
//   f391  "payer..."     (42.081s) — FIN DU PLAN, coupe nette sur la suspension (choix d'Aziz).
//   ⛔ La frontiere se pose APRES la fin du mot, jamais sur son ancre (lecon du segment 2, ou
//      "SOUVERAINS" avait ete coupe net a son attaque).
// Le plan 4 (la faille) prend le relais pour porter "CREUSER" (45.740s), dernier mot de l'episode.
//
...
const Floater: React.FC<{ it: FloatItem; frame: number; appear: number }> = ({
  ...
  // AGITATION : sur "negocient" (f322) la parole s'emballe — les papiers derivent plus vite et plus
  // large. Ce n'est pas un tremblement decoratif : c'est le geste qui dit que la negociation s'agite
  // pendant que, de l'autre cote, la meche continue d'avancer sans rien dire.
  const agit = 1 + 1.35 * Math.max(0, Math.min(1, (frame - 300) / 60));
  const spd = it.speed * (1 + 0.9 * Math.max(0, Math.min(1, (frame - 300) / 60)));
  const dx = it.drift * agit * Math.sin(frame * spd + it.phase);
  const dy = it.drift * 0.72 * agit * Math.cos(frame * spd * 0.83 + it.phase * 1.7);
...
export const GazoducActe5Negocier: React.FC = () => {
  ...
  const drillDescent = interpolate(frame, [80, 391], [0, 64], {
  ...
  const crackGrow = interpolate(frame, [70, 380], [0, 1], {

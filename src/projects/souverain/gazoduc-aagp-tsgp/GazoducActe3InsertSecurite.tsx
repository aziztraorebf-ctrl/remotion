...
//
// V4 (2026-08-14) — PORTAGE sur le nouveau décor SVG jugé visuellement supérieur par Aziz
// ("quasiment identiques à l'image Gemini de référence") : `GazoducAeroportFable5Test.tsx`
// (Fable 5 mode MAX, R&D 2026-08-13). Toute la logique de timing/beats/extinction/climax de la V3
// est CONSERVÉE À L'IDENTIQUE (deathFlicker, tourStaccado, darkenOverlay, climaxFilterOpacity,
// vie ambiante coupée nette à l'extinction) — seule la géométrie du décor change. Détail :
// memory/episodes/souverain/gazoduc-aagp-tsgp/STATUS.md § Segment B.
//
// Historique V3 (2026-08-07/08) conservé pour mémoire :
...
//   3. Caméra qui RESPIRE en continu (micro-zoom + micro-parallaxe ciel/sol) sur TOUTE la durée.
//   4. GATE TRANCHÉ AZIZ : décor gardé EN FILIGRANE (opacité résiduelle très faible) pendant le climax.
//   5. Manche à air : bruit composé (2 fréquences sinus superposées).
//   6. Vie ambiante (radar/véhicule/avion) qui vit en parallèle, coupée NETTE à l'extinction —
//      le nouveau décor a déjà véhicule+avion intégrés (sans chauffeur/pilote), le radar a ses
//      propres ondes radio déjà dessinées statiques (pas de rotation ajoutée, redondant visuellement).
//
// ⛔ Contrainte éditoriale préservée strictement : AUCUNE figuration humaine (vérifié sur le nouveau
// SVG — véhicule et avion sont vides, "sans chauffeur"/"sans pilote").
...
// ===== Géométrie reprise telle quelle du décor GazoducAeroportFable5Test.tsx =====
...
// Index de la fenêtre "entrée" (dernière du rang, cf FENETRES_TERMINAL[FENETRE_ENTREE_IDX] V3) —
// utilisé pour l'opacité du panneau vitré d'entrée, cohérent avec la dernière fenêtre du rang.
const FENETRE_ENTREE_IDX = TERMINAL_WIN_X.length - 1;
...
const PIANO_KEYS = [660, 745, 830, 915, 1000, 1085, 1170, 1255, 1340].map((xb) => {
  const vx = 1067.5;
  const shrink = (x: number) => x + (vx - x) * 0.135;
  const x2 = xb + 52;
...
// ===== Extinction avec vacillement organique (V3, inchangé) — chaque feu VACILLE avant de s'éteindre,
// puis chute nette (pas un fondu doux). =====
function deathFlicker(frame: number, deathFrame: number, seed: number): number {
  ...
  if (frame < deathFrame) {
    ...
    return 0.3 + 0.7 * Math.abs(Math.sin(t * 2.4 + seed));
  }
  return interpolate(frame, [deathFrame, deathFrame + 4], [1, 0], clampB);
}
...
export const GazoducActe3InsertSecurite: React.FC = () => {
  ...
  // ===== Caméra qui RESPIRE en continu — micro-zoom + micro-parallaxe ciel/sol (inchangé V3). =====
  ...
  const nFeux = RUNWAY_LIGHT_T.length;
  const feuxOffAt = (idx: number) => Math.round(interpolate(
    idx, [0, nFeux - 1],
  ...
  // Vie ambiante (véhicule/avion déjà dans le décor) : pleinement visible pendant la phase normale,
  // coupée NETTE à extinctionStart (4f, pas un fondu — cohérent avec le reste du décor).
  const ambientLifeOpacity = interpolate(frame, [extinctionStart - 4, extinctionStart], [1, 0], clampB);

  const feuOpacity = (idx: number, seed: number) => deathFlicker(frame, feuxOffAt(idx), seed);
  ...
  const climaxFilterOpacity = frame >= B.trenteCinqStart - 6
    ? Math.max(0.06, 1 - darkenOverlay)
    : 1 - darkenOverlay;

  // Avion au sol qui roule lentement en tout début de segment (0-9s) — mouvement d'aéroport nocturne
  // banal, JAMAIS un décollage/atterrissage. Le décor place l'avion à (890,790) au repos ; on ajoute
  // un déplacement RELATIF par-dessus (delta, pas une position absolue) pour rester compatible avec
  // le transform déjà présent dans le SVG source (`translate(890 790) scale(1.05)`).
  const avionLocal = frame;
  const avionDX = interpolate(avionLocal, [0, S(9)], [230, -250], clampB);
  const avionDY = interpolate(avionLocal, [0, S(9)], [2, -42], clampB);
  const avionTaxiOp = interpolate(avionLocal, [0, S(1), S(7.5), S(9)], [0, 1, 1, 0], clampB);

  // Véhicule de piste — gyrophare qui clignote à son PROPRE rythme (déphasé des feux de piste).
  const gyroPhase = (Math.sin(frame * 0.35) + 1) / 2;
  const gyroOn = gyroPhase > 0.72;

  return (
    <AbsoluteFill style={{ opacity: globalFade, background: "#060d24" }}>
      <svg width={1920} height={1080} viewBox="0 0 1920 1080" style={{ transform: `translate(${shakeX}px, ${shakeY}px) scale(${breathScale})`, transformOrigin: "50% 50%", opacity: sceneReveal }}>
        ...
        {/* ================= CIEL — couche LOINTAINE de la parallaxe ================= */}
        <g transform={`translate(${skyParallax} 0)`}>
          ...
          <g id="lune">
            ...
            <path d="M 1701 133 A 57 57 0 0 1 1621 214 A 66 66 0 0 0 1701 133 Z" fill="#7e93b6" opacity={0.35} />
          </g>
        ...
        {/* ================= SOL / PISTE / ARCHITECTURE — couche PROCHE, filigrane au climax ========= */}
        <g transform={`translate(${groundParallax} 0)`} opacity={climaxFilterOpacity}>
          ...
          <g id="routes">
            ...
            {[1300, 1352, 1404, 1456, 1508, 1560, 1612].map((x, i) => (
              <polygon key={`zeb-${i}`} points={`${x},886 ${x + 30},885 ${x + 34},842 ${x + 5},843`} fill="#c3cfe0" opacity={0.6} />
            ))}
          </g>
          ...
          {/* ================= BALISAGE LUMINEUX — extinction deathFlicker par index ================= */}
          <g id="balisage">
            {RUNWAY_LIGHT_T.map((t, i) => {
              const rP = 2.6 + 4.6 * t;
              const rH = 8 + 14 * t;
              const op = feuOpacity(i, i * 1.3);
              if (op <= 0.01) return null;
              return (
                <g key={`rwl-${i}`} opacity={op}>
            ...
            {TEAL_LEFT.map(([x, y], i) => {
              const op = feuOpacity(Math.floor((i / TEAL_LEFT.length) * nFeux), i * 2.1);
              if (op <= 0.01) return null;
              return <g key={`tl-${i}`} opacity={op}><TealLight cx={x} cy={y} rHalo={10.5 - i * 0.6} rPoint={3.8 - i * 0.22} /></g>;
            })}
            {TEAL_RIGHT.map(([x, y], i) => {
              const op = feuOpacity(Math.floor((i / TEAL_RIGHT.length) * nFeux), i * 1.6);
              if (op <= 0.01) return null;
              return <g key={`tr-${i}`} opacity={op}><TealLight cx={x} cy={y} rHalo={10 + i * 1.1} rPoint={3.6 + i * 0.4} /></g>;
            })}
            {ROAD_LEFT_AMBER.map((x, i) => {
              const op = feuOpacity(nFeux - 1, i * 0.9);
              if (op <= 0.01) return null;
              return <AmberLight key={`rla-${i}`} cx={x} cy={832 + i * 1.2} rHalo={13} rPoint={4.5} op={op} />;
            })}
            {ROAD_LEFT_TEAL.map((x, i) => {
              const op = feuOpacity(nFeux - 1, i * 1.1);
              if (op <= 0.01) return null;
              return <g key={`rlt-${i}`} opacity={op}><TealLight cx={x} cy={880} rHalo={10} rPoint={3.6} /></g>;
            })}
            {ROAD_RIGHT_AMBER.map((x, i) => {
              const op = feuOpacity(nFeux - 1, i * 0.7);
              if (op <= 0.01) return null;
              return <AmberLight key={`rra-${i}`} cx={x} cy={838 - i * 1} rHalo={13} rPoint={4.5} op={op} />;
            })}
          </g>

          {/* ================= PROJECTEURS (mât gauche) — coupés à l'extinction ================= */}
          <g id="projecteurs" opacity={1 - darkenOverlay}>
          ...
          {/* ================= TERMINAL — extinction fenêtre par fenêtre ================= */}
          <g id="terminal">
            <ellipse cx={400} cy={722} rx={300} ry={26} fill="url(#af_warm_pool)" opacity={0.65 * (1 - darkenOverlay)} />
            ...
            <rect x={292} y={688} width={100} height={28} fill="#ffd98a" opacity={0.92 * terminalOpacity(FENETRE_ENTREE_IDX)} />
            ...
            <ellipse cx={342} cy={722} rx={95} ry={12} fill="#ffd98a" opacity={0.25 * terminalOpacity(FENETRE_ENTREE_IDX)} />
            <g id="reflets_terminal" opacity={0.85}>
              {TERMINAL_WIN_X.map((x, i) => (
                <rect key={`twr-${i}`} x={x + 1} y={718} width={26} height={44 - (i % 3) * 8} fill="url(#af_win_refl)" opacity={terminalOpacity(i)} />
              ))}
              <rect x={292} y={718} width={100} height={52} fill="url(#af_win_refl)" opacity={0.9 * terminalOpacity(FENETRE_ENTREE_IDX)} />
            </g>
          </g>

          {/* ================= TOUR DE CONTROLE — tourStaccado sur cabine+radar+parabole ============= */}
          <g id="tour_controle">
            <ellipse cx={880} cy={376} rx={132} ry={72} fill="url(#af_cab_halo)" opacity={tourStaccado} />
            ...
            <polygon points="840,400 920,400 934,348 826,348" fill="url(#af_cab_glass)" opacity={tourStaccado} />
            {[0.22, 0.42, 0.61, 0.8].map((f, i) => {
              const xb = 840 + 80 * f;
              const xt = 826 + 108 * f;
              return <path key={`mul-${i}`} d={`M ${xb} 400 L ${xt} 348`} stroke="#7a5a30" strokeWidth={3} opacity={0.85 * tourStaccado} fill="none" />;
            })}
            <path d="M 831 382 L 930 382" stroke="#7a5a30" strokeWidth={2} opacity={0.5 * tourStaccado} fill="none" />
            <polygon points="846,396 868,396 884,352 858,352" fill="#fff4d6" opacity={0.3 * tourStaccado} />
            <rect x={818} y={340} width={124} height={10} rx={3} fill="#1d3156" />
            <rect x={818} y={338} width={124} height={3} fill="#3a5580" opacity={0.7} />
            <g id="parabole" opacity={tourStaccado}>
            ...
            <g id="radar" opacity={tourStaccado}>
            ...
            <circle cx={880} cy={268} r={26} fill="url(#af_red_glow)" opacity={0.55 * beaconOpacity} />
            <circle cx={880} cy={267} r={7} fill="#ff5a4d" opacity={beaconOpacity} />
            <ellipse cx={880} cy={720} rx={110} ry={13} fill="url(#af_warm_pool)" opacity={0.4 * (1 - darkenOverlay)} />
          </g>

          {/* ================= VEHICULE DE PISTE — gyrophare propre rythme, coupé à l'extinction ===== */}
          <g id="vehicule_piste" transform="translate(742 776)" opacity={ambientLifeOpacity}>
            ...
            {/* amber beacon on cab roof — clignote à SON rythme propre (gyroOn), déphasé des feux de piste */}
            <circle cx={16} cy={-19} r={9} fill="url(#light_glow)" opacity={gyroOn ? 0.7 : 0.15} />
            <rect x={13.5} y={-19.5} width={5} height={3.5} rx={1.2} fill={gyroOn ? "#ffb020" : "#5a3a1a"} />
          ...
          {/* ================= AVION AU ROULAGE — translate relatif par-dessus la position de repos === */}
          <g
            id="avion"
            transform={`translate(${890 + avionDX} ${790 + avionDY}) scale(1.05)`}
            opacity={ambientLifeOpacity * avionTaxiOp}
          >
          ...
          {/* ================= MANCHE A AIR (couleurs Niger) — rotation bruit composé, coupée au climax = */}
          <g id="manche_air">
            ...
            <circle cx={1670.5} cy={770} r={13} fill="url(#light_glow)" opacity={0.8 * (1 - darkenOverlay * 0.6)} />
            <circle cx={1670.5} cy={770} r={4} fill="#ffd98a" opacity={1 - darkenOverlay * 0.6} />
            <g id="manche_cone" transform={`rotate(${vaneAngle} 1670.5 786)`} opacity={1 - darkenOverlay * 0.6}>
        ...
        <rect width={1920} height={1080} fill="#04081a" opacity={darkenOverlay} />

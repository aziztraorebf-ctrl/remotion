...
lettrage("e1-titre-lettres", "Step one title", 34, M, 130, TXT1, face=2)
w('</g>', 2)
...
lettrage("e1-sous-titre-lettres", "STEP ONE SUBTITLE", 13, M, 168, TXT2, face=5, suivi=2.2)
w('</g>', 2)
...
lettrage("e1-section-lettres", "SECTION LABEL", 13, M, 366, TXT2, face=5, suivi=2.2)
w('</g>', 2)
...
lettrage("e1-banniere-ligne-1", "Banner headline", 15, M + 72, BY + 37, TXT1, face=2)
lettrage("e1-banniere-ligne-2", "Banner supporting line", 12, M + 72, BY + 58, TXT2, face=7)
...
lettrage("e2-titre-lettres", "Step two title", 30, M, 128, TXT1, face=2)
w('</g>', 2)
...
lettrage("e2-etape-1-texte", "First action to take", 14, M + 40, 181, TXT1, face=5)
w('</g>', 2)
...
for k, (rid, coul, droite, libelle, coul_txt) in enumerate(rangs):
    ...
    # Tant qu il vivait dans <g id="ecran-reglages">, il arrivait avec le
...
lettrage("e2-etape-2-texte", "Then the second step", 14, M + 40, EY + 5, TXT1, face=5)
w('</g>', 1)
...
lettrage("e2-apercu-titre", "Notification title", 15, M + 84, AY + 43, TXT1, face=2)
lw_now = largeur("now", 11, face=7)
lettrage("e2-apercu-heure", "now", 11, M + 396 - lw_now, AY + 42, TXT2, face=7, op=op_txt("0.8"))
lettrage("e2-apercu-corps-1", "Notification detail line", 12, M + 84, AY + 64, TXT2, face=7)
...
lettrage("e2-apercu-corps-2", "Notification second line", 12, M + 84, AY + 82,
...
lettrage("e2-note-ligne-1", "Footnote first line", 12, M + 72, NY + 32,
         TXT2, face=7, op="0.9")
lettrage("e2-note-ligne-2", "Footnote second line", 12, M + 72, NY + 50,
...
# Coordonnees absolues de planche : l ecran 2 est a translate(E2X 50).
w('')
w('<!-- === cadre de designation : <g> de 1er niveau, anime separement === -->')
w(f'<g id="e2-cadre" transform="translate({E2X + M + 12} {50 + CADRE_Y_LOCAL + 8})">', 1)
w(f'<rect id="e2-cadre-halo" x="0" y="0" width="396" height="{RANG_H - 16}" '
  f'rx="14" fill="{ACC}" opacity="0.10"/>', 2)
w(f'<rect id="e2-cadre-trait" x="0" y="0" width="396" height="{RANG_H - 16}" '
  f'rx="14" fill="none" stroke="{ACC}" stroke-width="2" opacity="0.95"/>', 2)
w('</g>', 1)

_RANG_ACTIF = OPTION_ACTIVE - 1
TOG_OFF_X = E2X + M + 348
TOG_OFF_Y = 50 + CY0 + _RANG_ACTIF * RANG_H + 22
TOG_ON_X  = E2X + M + 348
TOG_ON_Y  = 50 + CY0 + _RANG_ACTIF * RANG_H + 22

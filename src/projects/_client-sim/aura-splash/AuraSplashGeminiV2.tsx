...
//
/**
 * AURA splash — VERSION GEMINI V2 (choix d'Aziz, 2026-09-01).
 *
 * Identique a la V1 SAUF le rythme d'arrivee de `ura` (cf. CASCADE).
 * La V1 est conservee telle quelle a cote — on ne remplace pas, on ajoute.
 *
 * ⚠️ Le « gel apres 2,1 s » envisage n'a PAS ete applique : mesure faite,
 * le logo ne se deplace que de 0,8 px sur les 0,9 dernieres secondes
 * (grandit de 1 %). Ce qui change en fin de piece est le FOND qui
 * s'eclaircit (luminance 33 -> 51), et c'est le coeur du concept.
 * Il n'y avait donc rien a geler.
 *
 ...
 *  3. ⭐ MODIFIE EN V2 : arrivee des lettres en decalages IRREGULIERS courts
 *     (0 / 30 / 90 ms) au lieu de la cascade reguliere de 150 ms. translateY
 *     +15px -> 0 conserve. C'est la SEULE difference avec la V1.
 *  4. Micro-scale continu 1 -> 1.02 jusqu'a la fin. Mesure : effet reel de
 *     0,8 px de deplacement — imperceptible, conserve tel quel.
 *
...
/**
 * `u`, `r`, `a` — decalages IRREGULIERS et COURTS (seule modification vs V1).
 *
 * POURQUOI : la V1 suivait Gemini a la lettre — cascade reguliere de 150 ms.
 * GPT et Grok, independamment l'un de l'autre, designent exactement ce rythme
 * comme le tic « corporate » a eviter (« A puis U puis R comme un logo
 * d'entreprise »). Le brief client interdit explicitement le registre SaaS.
 * On garde donc TOUT le concept de Gemini et on casse la seule regularite :
 * ecarts inegaux (0 / 60 / 30 ms) au lieu de 150 ms constants.
 */
const CASCADE = [
  { key: "u", path: AURA_PATHS.uTrait, delay: 0 },
  { key: "a", path: AURA_PATHS.anneau, delay: 0.03 },
  { key: "a2", path: AURA_PATHS.jambage, delay: 0.03 },
  { key: "r", path: AURA_PATHS.rTrait, delay: 0.09 },
] as const;

export const AuraSplashGeminiV2: React.FC = () => {

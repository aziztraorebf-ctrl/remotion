#!/usr/bin/env python3
"""Genere `../planche.ts` depuis `planche-ui.svg` (source de verite).
...
svg = (ICI / "planche-ui.svg").read_text(encoding="utf-8")
...
print(f"planche.ts <- planche-ui.svg : {len(blocs)} groupes ({', '.join(g for g, _ in blocs)})")

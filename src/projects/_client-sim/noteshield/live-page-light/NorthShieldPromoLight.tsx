...
/**
 * NorthShieldPromoLight — VARIANTE CLAIRE (light mode SaaS) du meme film.
 *
 * But du test (demande Aziz 2026-08-20) : verifier que le pipeline shotcraft tient sur
 * une page web NORMALE (fond blanc), et pas seulement sur notre registre sombre habituel.
 * Aucun composant ne change : seules la capture source et la palette varient. Si ca tient,
 * le pipeline est reellement agnostique du design du client — ce qui est la condition pour
 * le vendre a n'importe quel SaaS.
 *
 * ⛔ Toutes les coordonnees de l'etat filtre sont MESUREES au pixel sur la plaque claire
 * (scan du cercle de score rouge), jamais deduites de la version sombre : la page claire a
 * un tableau encadre, donc des offsets differents (row0 y=234 vs 247, search y=107 vs 138).
 *
 * Base : NorthShieldPromoV4 : cette fois on UTILISE leurs composants, on ne les
 * reimplemente pas.
 *
...
import { PageCam, type CamKey } from "../live-page/shotcraft-lib/PageCam";
import { FlashCut } from "../live-page/shotcraft-lib/FlashCut";
import layout from "./live-layout.json";

export const NS_LIGHT_FPS = 30;
export const NS_LIGHT_FRAMES = 780;
...
const NAVY = "#FBFBFD";        // fond page claire
const IVORY = "#0F1729";       // encre (le texte devient sombre)
const CYAN = "#0B63F6";        // accent bleu SaaS
const RED = "#D93A47";
const MUTED = "#6B7688";
const MONO = "'IBM Plex Mono', monospace";
const DISPLAY = "'Space Grotesk', 'Inter', sans-serif";

const FULL = "_client-sim/noteshield/live-light/dashboard-full.png";
const EMPTY = "_client-sim/noteshield/live-light/dashboard-empty.png";
const FILTERED = "_client-sim/noteshield/live-light/filtered-after.png";
    ...
    <div style={{ position: "absolute", left: x, top: y, transform: `scale(${1 / z})`, transformOrigin: "0 0", pointerEvents: "none" }}>
      ...
      <svg width={30} height={42} viewBox="0 0 24 34" style={{ display: "block" }}>
        <path d="M3 2 L3 26 L9.5 20.5 L13.5 30 L17.5 28 L13.5 19 L21 18.5 Z"
          fill="#1B2433" stroke="#FFFFFF" strokeWidth={1.6} strokeLinejoin="round"
  ...
  return (
    <PageCam src={EMPTY} pageH={PAGE_H} keys={keys}>
      ...
      <div style={{
        ...
        background: "#FFFFFF", border: `1px solid ${typed > 0 ? CYAN + "88" : "#0F1F38"}`,
 ...
 */
const FILT_ROW_CY = 641;   // mesure: centre du cercle de score
const FILT_SCORE_CX = 1324; // mesure: milieu des 2 bords de l'anneau (1289/1359)
const FILT_ROW_X = 97;
const FILT_ROW_Y = 577;    // mesure: haut de la bande rose
const FILT_ROW_W = 1726;
const FILT_ROW_H = 132;    // mesure: hauteur de la bande rose
...
export const NorthShieldPromoLight: React.FC = () => (
  <AbsoluteFill style={{ background: NAVY }}>
    ...
    <Sequence from={CUT_G} durationInFrames={NS_LIGHT_FRAMES - CUT_G}><Outro /></Sequence>
    ...
    <Sequence from={0} durationInFrames={NS_LIGHT_FRAMES}>

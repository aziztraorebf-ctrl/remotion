...
| `KeyBench` (**prouvé**, 2 usages : clés + appareils, 2026-08-26) | `_shared/_demos/keys/KeyBench.tsx` | "TRANCHER entre plusieurs variantes d'un même objet 3D sans se faire piéger par un angle flatteur isolé" — grille de N angles fixes côte à côte (`BENCH_ANGLES`), éclairage et échelle identiques, nom du modèle masqué (test aveugle). ⚠️ Son `BenchLights` est 100 % frontal : ajouter un contre-jour avant de juger un objet AJOURÉ. ⭐ 2e instance : `_demos/devices/DeviceBench.tsx` — le banc d'angles est un **patron pour tout objet 3D**, pas seulement les clés. |
...
| `PhoneModel` (**prouvé**, 6 usages, 2026-08-26) | `_shared/_demos/devices/PhoneModel.tsx` | "faire ressentir qu'un produit logiciel VIT sur un vrai appareil, et non dans un cadre plat" — smartphone 3D procédural (châssis chanfreiné, boutons, îlot caméra), géométrie pure, props `{rotationY, scale, screen}`, ZÉRO lumière/caméra/animation interne (la scène les fournit). Écran = plan `phone-screen-plane`, 1.39×3.012 (19.5:9) à z=0.106, où l'on branche un **material** texturé (UI ou vidéo). ⛔ PAS `<Html>` de drei pour l'écran : hors canvas, aucune occlusion au profil, désync d'une frame en headless. |
| `LaptopModel` (**prouvé**, 8 usages, 2026-08-26) | `_shared/_demos/devices/LaptopModel.tsx` | "un dashboard doit se lire comme un PRODUIT posé sur un bureau, et on veut pouvoir OUVRIR le capot ou tourner autour" — laptop 3D procédural (charnière barillet réelle, clavier data-driven ~78 touches sans glyphe). `lidAngle` est un **paramètre statique** (0.06 fermé → 1.85 ouvert) : le capot s'ouvre à l'image sans rien coder. Écran 16:10 = mesh `SCREEN_MESH_NAME` (2.88×1.80). ⭐ Touches quasi affleurantes (relief 0.012) : plus de relief lisait « vieux laptop ». ⭐ Alternative 2D : `_client-sim/noteshield/ui/LaptopMockup.tsx` (SVG Fable, 21 groupes nommés) — **règle de choix** dans `memory/doctrines/MOTEURS-VISUELS-ET-SOCLE.md` § OBJET 3D APLATI. |
| `GridBackdrop` + `GRID_TEAL`/`GRID_SLATE` (**prouvé**, 3 usages, 2026-08-26) | `_shared/_demos/devices/GridBackdrop.tsx` | "poser un objet quelque part SANS hériter des contraintes cachées d'une photo (échelle implicite, lumière figée, angle de vue, profondeur de champ)" — fond SVG « plan technique » entièrement **paramétré** : nappe radiale, grille à 2 pas par `<pattern>`, graduations qui se tracent, halo central qui remplace l'ombre portée, respiration 1,5 %. Props `{palette, cell, breathe}` → adaptable à la charte d'un client en 6 valeurs. ⚠️ Distinct de `GridBackground` (statique, à poser dans un `<svg>` du registre encre) : ici c'est un `AbsoluteFill` autonome et animé. |

...
| ⚠️ **Main-curseur, 3 poses** (repos / appui / pointe) — **GREFFE de structure** depuis un Lottie premium, PAS un dessin depuis image (corrigé 08-29). Anatomie approximative : index en tube droit sans phalange, 3 doigts repliés identiques — bon comme ICÔNE d'interface, ⛔ pas comme main | `elements/ui/main-curseur-3poses.svg` | — (doc inline) | Interface / démo produit mobile | Concours SVG 2026-08-28. ⛔ Registre où 5 modèles sur 5 échouent **sans référence** — lire `.claude/agent-memory/svg-dessinateur/ECHECS.md` avant d'en redemander une |
| ⭐ **Main-curseur greffée** (1 chemin continu, la plus légère) | `elements/ui/main-curseur-greffee.svg` | `memory/tools/banques-lottie-et-greffe.md` | Interface / démo produit mobile | ⛔⛔ **Licence Lottie Simple VIRALE** — portfolio uniquement, JAMAIS un livrable client exclusif |
| ⭐⭐ **COMMENT animer ces 2 mains** | — | `_client-sim/repro-redeem/MainGreffeeDemo.tsx` (démo) | Interface | ⛔ **Le doigt NE PLIE PAS** : 0 morphing mesuré sur 3 références de banque. L'illusion du tap vient du **déplacement de la main + de l'onde de contact**. Ne pas tenter d'articuler un doigt — c'est une itération perdue d'avance |

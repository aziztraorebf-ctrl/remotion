...
export type PoseInput = {
  ...
  moving?: boolean;   // true = marche ; false = arret (balancier eteint)
  bend?: number;      // 0 debout .. 1 penche (se baisser)
  armReach?: number;  // 0..1 extension du bras avant vers le sol-avant
};
...
export function computePose({ walkPhase = 0, moving = true, bend = 0, armReach = 0 }: PoseInput): Pose {
  ...
  const hipBob = moving ? Math.abs(Math.sin(phase)) * bobAmp * (1 - b) : 0;
  ...
  // bras avant (la main qui agit). angle 0 = vers le bas ; + = vers l'avant.
  const frontWalkDeg = swingDeg * 0.6;
  const frontReachDeg = 22; // vers le SOL-avant, independant du torse
  const frontArmDeg = frontWalkDeg * (1 - r) + frontReachDeg * r;

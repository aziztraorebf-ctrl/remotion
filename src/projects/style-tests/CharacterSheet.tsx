...
const Pierre: React.FC<CharProps> = ({ x, y, facing = "right", anim = "idle", frame, scale = 1, etat = "sain" }) => {
...
const Martin: React.FC<CharProps> = ({ x, y, facing = "right", anim = "idle", frame, scale = 1, etat = "sain" }) => {
  const flip = facing === "left" ? -1 : 1;
  const walk = frame * 0.1;
...
const Isaac: React.FC<CharProps> = ({ x, y, facing = "right", anim = "idle", frame, scale = 1, etat = "sain" }) => {
  const flip = facing === "left" ? -1 : 1;
  const walk = frame * 0.1;
  const isWalk = anim === "walk" && etat !== "mort";
...
const Guillaume: React.FC<CharProps> = ({ x, y, facing = "right", anim = "idle", frame, scale = 1.1, etat = "sain" }) => {
  const flip = facing === "left" ? -1 : 1;
  const walk = frame * 0.1;
  const isWalk = anim === "walk" && etat !== "mort";
  const isTalk = anim === "talk" && etat !== "mort";
...
const Agnes: React.FC<CharProps> = ({ x, y, facing = "right", anim = "idle", frame, scale = 1, etat = "sain" }) => {
  const flip = facing === "left" ? -1 : 1;
  const walk = frame * 0.1;
  const isWalk = anim === "walk" && etat !== "mort";
  const baseOpacity = etat === "mort" ? 0.55 : 1;
...
const Renaud: React.FC<CharProps> = ({ x, y, facing = "right", anim = "idle", frame, scale = 1, etat = "sain" }) => {
  const flip = facing === "left" ? -1 : 1;
  const walk = frame * 0.1;
  const isWalk = anim === "walk" && etat !== "mort";
  const baseOpacity = etat === "mort" ? 0.55 : 1;
...
// Carte de personnage avec etiquette
...
}> = ({ x, y, name, role, color, frame, appearFrame, children }) => {
  const localFrame = frame - appearFrame;
  const opacity = interpolate(localFrame, [0, 20], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const slideY = interpolate(localFrame, [0, 20], [30, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });

  const cardW = 280;
  const cardH = 340;

  return (
    <g transform={`translate(${x}, ${y + slideY})`} opacity={opacity}>
      {/* Fond carte parchemin */}
      <rect x={-cardW / 2} y={-cardH + 20} width={cardW} height={cardH} rx={8}
        fill={PARCHMENT_DARK} stroke={GOLD_DARK} strokeWidth={2} />
      {/* Bande couleur personnage en bas */}
      <rect x={-cardW / 2} y={20} width={cardW} height={32} rx={4}
        fill={color} stroke={INK} strokeWidth={1.5} />
      {/* Nom */}
      <text x={0} y={33} textAnchor="middle" fontSize={16}
        fontFamily="serif" fontWeight="bold" fill={PARCHMENT} letterSpacing={1}>
        {name}
      </text>
      {/* Role */}
      <text x={0} y={44} textAnchor="middle" fontSize={10}
        fontFamily="serif" fill={PARCHMENT} fontStyle="italic" opacity={0.85}>
        {role}
      </text>
      {/* Separateur */}
      <line x1={-cardW / 2 + 16} y1={-cardH + 80} x2={cardW / 2 - 16} y2={-cardH + 80}
        stroke={GOLD_DARK} strokeWidth={1} opacity={0.5} />
      {/* Personnage */}
      {children}
    </g>
  );
...
export const CharacterSheet: React.FC = () => {
  ...
  const enter = (start: number, dur = 20) =>
  ...
  // 6 personnages equidistants sur 1920px : marge 110px, espacement 340px
  // Guillaume scale=1.1 : compensation translateY = groundY * (1 - 1/1.1) = 74.5px
  const GUILLAUME_Y_OFFSET = groundY * (1 - 1 / 1.1);
  const CHARS = [
    { x: 160,  name: "Pierre",   role: "Le Laboureur",   color: "#5a4020", appearFrame: 8   },
    { x: 480,  name: "Martin",   role: "Le Prêtre",      color: LAPIS,     appearFrame: 20  },
    { x: 800,  name: "Isaac",    role: "Le Prêteur",     color: VERT,      appearFrame: 32  },
    { x: 1120, name: "Guillaume",role: "Le Seigneur",    color: VERMILLON, appearFrame: 44  },
    { x: 1440, name: "Agnès",    role: "La Guérisseuse", color: "#7a5828", appearFrame: 56  },
    { x: 1760, name: "Renaud",   role: "Le Médecin",     color: "#1a1a1a", appearFrame: 68  },
  ];
  ...
  return (
    <AbsoluteFill style={{ background: PARCHMENT }}>
      <svg width="1920" height="1080" viewBox="0 0 1920 1080" xmlns="http://www.w3.org/2000/svg">
        {/* Fond */}
        <rect width="1920" height="1080" fill={OCRE_PALE} opacity={0.3} />

        {/* Ciel lapis en haut */}
        <rect width="1920" height="200" fill={LAPIS} opacity={0.15} />

        {/* Titre */}
        <g opacity={enter(0, 30)}>
          <text x={960} y={65} textAnchor="middle" fontSize={26}
            fontFamily="serif" fill={GOLD_DARK} letterSpacing={4}>
            PERSONAE · VILLA SANCTI PETRI · MCCCXLVII
          </text>
          <line x1={150} y1={80} x2={1770} y2={80} stroke={GOLD} strokeWidth={1.5} opacity={0.6} />
          <circle cx={960} cy={80} r={5} fill={GOLD} />
        </g>

        {/* Sol dallage */}
        <g>
          <rect x={0} y={groundY} width={1920} height={260} fill={OCRE} />
          {Array.from({ length: 20 }, (_, col) =>
            Array.from({ length: 4 }, (_, row) => {
              const isAlt = (col + row) % 2 === 0;
              return (
                <rect key={`${col}-${row}`}
                  x={col * 96} y={groundY + row * 58}
                  width={96} height={58}
                  fill={isAlt ? OCRE_LIGHT : OCRE}
                  stroke={GOLD_DARK} strokeWidth={1} opacity={0.65}
                />
              );
            })
          )}
          <rect x={0} y={groundY} width={1920} height={5} fill={GOLD_DARK} opacity={0.5} />
        </g>

        {/* Les 6 personnages - tous en etat "sain" */}
        <g opacity={enter(8)}>
          <Pierre x={CHARS[0].x} y={groundY} frame={frame} facing="right" anim="idle" etat="sain" />
        </g>
        <g opacity={enter(20)}>
          <Martin x={CHARS[1].x} y={groundY} frame={frame} facing="right" anim="pray" etat="sain" />
        </g>
        <g opacity={enter(32)}>
          <Isaac x={CHARS[2].x} y={groundY} frame={frame} facing="right" anim="talk" etat="sain" />
        </g>
        {/* Guillaume scale=1.1 : translateY compense le decalage introduit par le scale SVG */}
        <g opacity={enter(44)} transform={`translate(0, ${GUILLAUME_Y_OFFSET})`}>
          <Guillaume x={CHARS[3].x} y={groundY - GUILLAUME_Y_OFFSET} frame={frame} facing="right" anim="idle" etat="sain" scale={1.1} />
        </g>
        <g opacity={enter(56)}>
          <Agnes x={CHARS[4].x} y={groundY} frame={frame} facing="right" anim="idle" etat="sain" />
        </g>
        <g opacity={enter(68)}>
          <Renaud x={CHARS[5].x} y={groundY} frame={frame} facing="left" anim="idle" etat="sain" />
        </g>

        {/* Etiquettes sous chaque personnage */}
        {CHARS.map((char, i) => {
          const localOpacity = interpolate(frame, [char.appearFrame + 12, char.appearFrame + 30], [0, 1], {
            extrapolateLeft: "clamp", extrapolateRight: "clamp",
          });
          return (
            <g key={i} opacity={localOpacity}>
              <rect x={char.x - 75} y={groundY + 12} width={150} height={38} rx={4}
                fill={char.color} stroke={INK} strokeWidth={1.5} />
              <text x={char.x} y={groundY + 26} textAnchor="middle" fontSize={12}
                fontFamily="serif" fontWeight="bold" fill={PARCHMENT} letterSpacing={1}>
                {char.name}
              </text>
              <text x={char.x} y={groundY + 40} textAnchor="middle" fontSize={10}
                fontFamily="serif" fill={PARCHMENT} fontStyle="italic" opacity={0.9}>
                {char.role}
              </text>
            </g>
          );
        ...
        {/* Separateurs verticaux entre personnages - 5 separateurs pour 6 zones */}
        {[320, 640, 960, 1280, 1600].map((sx, i) => (
          <g key={i} opacity={enter(80)}>
            <line x1={sx} y1={100} x2={sx} y2={groundY - 10}
              stroke={GOLD_DARK} strokeWidth={1} strokeDasharray="4,6" opacity={0.25} />
          </g>
        ))}

        {/* Bordure or */}
        <g opacity={enter(0, 40)}>
          <rect x={20} y={20} width={1880} height={1040} rx={10}
            fill="none" stroke={GOLD} strokeWidth={6} />
          <rect x={30} y={30} width={1860} height={1020} rx={8}
            fill="none" stroke={GOLD_DARK} strokeWidth={1.5} opacity={0.5} />
        ...
        {/* Sous-titre bas */}
        <text x={960} y={1062} textAnchor="middle" fontSize={11}
          fontFamily="serif" fill={GOLD_DARK} fontStyle="italic"
          opacity={enter(80)}>
          Six témoins du même malheur · Six réponses différentes · Una pestis, variae sortes
        </text>
      </svg>
    </AbsoluteFill>
  );

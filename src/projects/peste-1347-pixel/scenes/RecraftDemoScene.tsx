...
/**
 * Demo: Veo 3 animated background (from Recraft illustration) + light HUD overlay.
 * 10 seconds (300 frames at 30fps).
 * Veo handles ALL scene animation. Remotion only adds text HUD + CRT post-processing.
 */

export const RecraftDemoScene: React.FC = () => {
  ...
  const deathCount = Math.floor(
    interpolate(frame, [statStart, statStart + 90], [0, 25000000], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
    })
  );
  ...
  // -- Narrative text --
  const narrativeOpacity = interpolate(frame, [140, 160], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  ...
  return (
    ...
    >
      {/* LAYER 1: Veo 3 animated background - handles ALL scene animation */}
      <Video
        src={staticFile("assets/peste-pixel/backgrounds/veo3-animated-street.mp4")}
        style={{
          position: "absolute",
          width: "100%",
          height: "100%",
      ...
      {/* LAYER 2: Light text HUD only (no darken overlay, no SVG characters) */}

...
// ============================================================
// PIXEL WORLD V6 - PixelLab side-view characters replacing itch.io
//
// Based on V5 (camera, overlays, buildings, atmosphere all kept)
// CHANGES from V5:
//   1. PeasantNPC replaced by PixelLabNPC (individual frame PNGs)
//   2. No Math.sin bob - fixed anchoring at GROUND_H
//   3. Drop shadow ellipse on all NPCs
//   4. Fire repositioned to ground level
//   5. Math.floor on all interpolated scroll positions
//
...
const GROUND_H = 213; // Measured: cobblestone top at Y=895, bottom = 1080-895 = 185, +28 foot offset = 213

// ============================================================
// CAMERA SYSTEM (identical to V5)
// ============================================================
function useCamera(frame: number, _fps: number) {
...
// ============================================================
// Parallax Sky (Math.floor added for anti-subpixel)
// ============================================================
const ParallaxLayer: React.FC<{
  ...
  const scrollX = Math.floor(-(scrollPos * speed) % displayW);
...
// ============================================================
// Ground - AI-generated cobblestone (Math.floor added)
// ============================================================
const AI_GROUND_SRC = "assets/peste-pixel/town/extracted/ground-ai.png";
const AI_GROUND_W = 1536;
const AI_GROUND_DISPLAY_H = 160;

const Ground: React.FC<{ scrollPos: number }> = ({ scrollPos }) => {
  const scrollX = Math.floor(-(scrollPos * 1.5) % AI_GROUND_W);
  const tilesNeeded = Math.ceil(1920 / AI_GROUND_W) + 2;
    ...
    <>
      ...
      >
        {Array.from({ length: tilesNeeded }, (_, i) => (
          <Img
            key={i}
            src={staticFile(AI_GROUND_SRC)}
            style={{
              position: "absolute",
              left: scrollX + i * AI_GROUND_W,
              bottom: 0,
              width: AI_GROUND_W,
              height: AI_GROUND_DISPLAY_H,
              imageRendering: "pixelated",
...
// ============================================================
// Building (Math.floor on scrolledX)
// ============================================================
...
const Building: React.FC<{
  ...
  const scrolledX = Math.floor(x - scrollPos * 1.5);
...
// ============================================================
// PixelLab NPC - Replaces PeasantNPC (itch.io spritesheet)
//
// Uses individual frame PNGs from PixelLab side-view characters.
// Anchoring: bottom = GROUND_H - FOOT_OFFSET_PX
//   PIL measurement: lowest opaque row = 56/64, transparent padding = 7px native
//   At 4x display scale: 28px offset -> feet land exactly on ground line
// Drop shadow ellipse for grounding.
// ============================================================
// Transparent bottom padding (pixels below lowest opaque pixel), at 4x display scale.
// Measured via PIL: all 4 sprites have 7px native = 28px displayed.
const FOOT_OFFSET_PX = 7 * 4; // 28px at 4x scale

const PIXELLAB_CHARS = {
  "peasant-man": "assets/peste-pixel/pixellab/side-view/characters/peasant-man-side/animations/walking",
  "peasant-woman": "assets/peste-pixel/pixellab/side-view/characters/peasant-woman-side/animations/walking",
  "monk": "assets/peste-pixel/pixellab/side-view/characters/monk-side/animations/walking",
  "merchant": "assets/peste-pixel/pixellab/side-view/characters/merchant-side/animations/walking",
} as const;

type CharName = keyof typeof PIXELLAB_CHARS;

const FRAME_COUNT = 6;
const NATIVE_SIZE = 64;

// Two ground levels for depth:
// SIDEWALK (trottoir) = +35px above street = NPCs walking on raised pavement in front of buildings
// STREET (rue) = 0 = NPCs walking on the main road (lower, appears closer/in front)
const SIDEWALK_OFFSET = 35;
const STREET_OFFSET = 0;

const PixelLabNPC: React.FC<{
  name: CharName;
  x: number;
  direction: "east" | "west";
  frame: number;
  ...
  frameOffset?: number;
  groundOffset?: number;
}> = ({ name, x, direction, frame, scrollPos, displayScale = 4, frameOffset = 0, groundOffset = STREET_OFFSET }) => {
  const scrolledX = Math.floor(x - scrollPos * 1.5);
  const spriteFrame = (Math.floor((frame + frameOffset) / 5) % FRAME_COUNT);
  const basePath = PIXELLAB_CHARS[name];
  const framePath = `${basePath}/${direction}/frame_00${spriteFrame}.png`;
  const displaySize = NATIVE_SIZE * displayScale;
  ...
  return (
    <div
      style={{
        position: "absolute",
        left: scrolledX,
        bottom: GROUND_H - FOOT_OFFSET_PX + groundOffset,
        zIndex: 18,
        width: displaySize,
        height: displaySize,
      }}
...
// ============================================================
// Running Rats (Math.floor on position)
// ============================================================
  ...
  return (
    <>
      {rats.map((rat, i) => {
        ...
        const rx = Math.floor(rat.startX + elapsed * rat.speed);
...
// ============================================================
// Fire FX (repositioned to ground level, Math.floor added)
// ============================================================
...
// ============================================================
// DATA OVERLAY: Propagation chart (identical to V5)
// ============================================================
...
// ============================================================
// DATA OVERLAY: Population stat card (identical to V5)
// ============================================================
...
// MAIN COMPOSITION
// ============================================================
export const PixelWorldV6: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* === LAYER 5: PixelLab NPCs (replaces itch.io peasants) === */}
        {/* groundOffset: SIDEWALK_OFFSET = trottoir devant batiments, STREET_OFFSET = rue principale */}
        <PixelLabNPC name="peasant-man" x={570} direction="east" frame={frame} scrollPos={scrollPos} frameOffset={0} groundOffset={SIDEWALK_OFFSET} />
        <PixelLabNPC name="peasant-woman" x={620} direction="west" frame={frame} scrollPos={scrollPos} frameOffset={12} groundOffset={STREET_OFFSET} />
        <PixelLabNPC name="monk" x={1100} direction="east" frame={frame} scrollPos={scrollPos} frameOffset={7} groundOffset={SIDEWALK_OFFSET} />
        <PixelLabNPC name="merchant" x={1580} direction="west" frame={frame} scrollPos={scrollPos} frameOffset={20} groundOffset={STREET_OFFSET} />
        <PixelLabNPC name="peasant-man" x={2200} direction="east" frame={frame} scrollPos={scrollPos} frameOffset={15} groundOffset={SIDEWALK_OFFSET} />
        <PixelLabNPC name="peasant-woman" x={2700} direction="west" frame={frame} scrollPos={scrollPos} frameOffset={3} groundOffset={STREET_OFFSET} />

        {/* === LAYER 6: Rats === */}
        <RunningRats frame={frame} scrollPos={scrollPos} />

        {/* === LAYER 7: Fire FX (repositioned to ground) === */}
        <FireFX x={540} bottomOffset={-8} frame={frame} scrollPos={scrollPos} variant={1} size={3} />
        <FireFX x={1140} bottomOffset={-8} frame={frame} scrollPos={scrollPos} variant={2} size={4} />
        <FireFX x={1720} bottomOffset={-8} frame={frame} scrollPos={scrollPos} variant={3} size={3} />

        {/* === LAYER 8: Night atmosphere === */}
        <div
          style={{
            position: "absolute",
            inset: 0,

...
| Kimi (review + vision→SVG) | **`kimi-k3` UNIQUEMENT** via Moonshot API — importer `KIMI` de `scripts/tools/api_models.py`. ⛔ k2.5 / k2.6 / k26 PÉRIMÉS (décision Aziz 2026-08-20 : une seule variante, la plus récente). ✅ `da-brief.py` MIGRÉ vers k3 le 2026-09-08 (borne `reasoning.max_tokens` posée, replis supprimés) — plus aucun script en contournement. |
...
**Absolus** : phase 0 SCAN COMPOSANTS-INDEX gate bloquant · 2 appels Gemini MAX · Tailwind partout (exception SVG/animations) · R1 = max 8s sans changement visuel · self-review ≥19/25 avant Gemini · upload (Artifact par défaut, cf. § Communication mobile) + ntfy avant toute présentation.
...
- **Tout render → présenter par un LIEN, jamais un chemin local.** ⭐⭐⭐ **ARTIFACT CLAUDE = LE DÉFAUT** (images, pages HTML, ET vidéos compressées — plafond **16 Mo par page**). Je le crée et le mets à jour moi-même, sans script ni hôte externe, et Aziz l'ouvre sur mobile.
- ⭐⭐ **UNE PAGE ARTIFACT PAR SUJET, pas un lien par livrable.** Dès qu'un sujet implique des images (nouvelle vidéo, reverse engineering, comparatif), créer SA page et l'enrichir toute la session — même URL redéployée. **Le lien vit dans le STARTER de reprise du sujet, jamais dans `MEMORY.md`** (sinon des dizaines d'URL engorgent la mémoire active).
- **Exceptions, dans cet ordre** : rendu full HD > 16 Mo (épisode complet : nos assemblages font 230-400 Mo, nos beats 2,4 Mo de médiane) → **Vercel Blob** (`scripts/tools/upload-to-blob.py`) · Blob indisponible ou hors quota → **catbox → Litterbox** · Artifact indisponible (image ou vidéo) → **catbox → Litterbox**. Toujours vérifier `content-length` après un upload externe (catbox renvoie HTTP 200 + `content-length: 0` en cas de faux succès). Détail : `.claude/.../memory/feedback_upload-hosts-fallback.md`.

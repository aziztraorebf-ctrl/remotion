...
| **Ecrire / valider TOUT script (Short, Mid-form, Atlas, War-Map, tout format) — niveau oral OBLIGATOIRE** | `memory/doctrines/DOCTRINE-SCRIPT-UNIFIEE.md` ⭐ **SOURCE DE VERITE UNIQUE (couche orale universelle)** — 15 regles en 3 familles : A. CLARTE (phrase <22 mots, test "et alors ?", verbes avec sujet, tampons, question centrale <20s, liaisons, vulgarisation), B. INCARNATION/Tremblay (registre on/nous jamais "tu", pas de CTA dans le hook, connecteur de presence par acte, paradoxe tenu), C. RETENTION (boucle ouverte, escalade emotionnelle, rythme syncope). Checklist audio lock complete. S'applique a TOUTES les chaines AVANT audio lock. NB : la STRUCTURE narrative (le QUOI) reste dans les doctrines de pilier (DOCTRINE-SOUVERAIN, ATLAS-PLAYBOOK, WARMAP-*). | — |
| **Coder une scene/beat War-Map (montrer un changement de territoire, une avancee, une chute) — AVANT TOUT CODE** | `memory/doctrines/WARMAP-GRAMMAIRE-CAUSALE.md` ⭐⭐ **NON-NEGOTIABLE** — la regle CAUSE avant EFFET (un acteur AGIT -> le territoire change) + catalogue des 5 techniques causales validees (avancee jetons+sillage, chute base 3 temps, donnee qui se MONTRE par remplissage, contour flash, casser la grammaire pour acteur different). Evite le "bordel confus" (etats qui poppent = rejete). Ref : Sahel P2. | — |
